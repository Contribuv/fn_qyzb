...
### v1.0.11 (2026-08-27)
- 修复：聊天室发送多张图片后未自动滚动到底的问题（图片异步加载完成后补一次滚动）
- 修复：暗色主题下移动端登录页标题「千盈助播」文字颜色异常（深底深字不可见）
- 更新：主题切换按钮尺寸加大，前台/后台点击命中区域更准确

### v1.0.11 (2026-08-28)
- 修复：发送多张图片后聊天区未自动滚动到底的问题（图片加载完成后再次滚动）
- 修复：暗色主题下移动端登录页标题文字颜色显示异常
- 优化：静态资源缓存版本号改为自动跟随版本号，升级时无需再逐个模板修改

### v1.0.10 (2026-08-27)
- 更新：全新应用图标（3D 玻璃质感、连续曲率圆角、透明背景、高光学细节），全端统一 1024×1024 尺寸
- 更新：favicon、桌面图标等全部采用最新 logo

### v1.0.9 (2026-08-26)
- 改版：网关（反向代理）页面 PC + 移动端全新设计
- 新增：PC 端 macOS 风格侧边栏，深浅色主题自动跟随系统并可手动切换
- 新增：移动端底部固定 Tab 栏 + 「更多」抽屉（实时日志 / 帮助说明 / 切换主题 / 版本）
- 修复：暗色主题下聊天输入框聚焦时仍为白底的问题

### v1.0.8 (2026-08-25)
- 新增：聊天室图片消息功能（Web 端上传 + 桌面端提图窗口）
- 新增：Web 端单次上传最多 10 张图片，支持 jpg/jpeg/png/gif/webp，单张上限 5MB
- 新增：Web 端图片消息以网格布局展示，支持点击查看大图
- 新增：桌面端收到图片消息后在主显示器居中弹出独立提图窗口（标题：千盈助播系统-提图窗口）
- 新增：提图窗口置顶显示，支持多图左右翻页、放大缩小（按钮/滚轮/键盘）、双击重置
- 新增：图片全部下载并解码完成后才弹出窗口，避免逐张加载闪烁
- 优化：登出/切换房间时自动关闭提图窗口并清理下载线程
- 优化：历史图片消息不再触发弹窗，仅实时新图片触发

### v1.0.7 (2026-07-18)
- 修复：系统重启后残留 socket 文件导致 status() 误判应用在运行，飞牛系统不调用 start()
- 修复：start() 启动前不清理残留 socket 文件
- 优化：status() 移除 socket 文件存在性检查，仅检查 PID 和 TCP 端口

### v1.0.6 (2026-07-18)
- 修复：系统重启后统一网关不可用（/var 目录被清空导致 socket 创建失败）
- 优化：移除不可靠的符号链接方案，改为多路径 socket 创建
- 优化：Go 程序增加默认 socket 路径，不依赖环境变量
- 优化：socket 目录创建失败时打印详细错误信息便于排查
